...
c = conn.cursor()
conn2 = sqlite3.connect('nom_prenom.db')
conn2.text_factory = str    #Corrige un bug provoqué par les accents français
w = conn2.cursor()
conn3 = sqlite3.connect('pseudo.db')
z = conn3.cursor()

c.execute("""CREATE TABLE IF NOT EXISTS chien(id_chien INT, sexe INT, id_race INT, age DATE, taille INT, pedigree INT, id_vendeur INT, prix DECIMAL(6,2), PRIMARY KEY(id_chien,id_race,id_vendeur),
...
def random_particulier(n):
    ...
    z.execute("""SELECT COUNT(*) FROM pseudo""")
    lenpseudo = (z.fetchone())[0]
    ...
    for i in range(lenvend+1,lenvend+n+1):
        ...
        if hasard <= 10:
            ...
            z.execute("""SELECT pseudo FROM pseudo WHERE id_pseudo = ?""", (randint(1,lenpseudo),))
            nom = (z.fetchone())[0]
...
def random_dog(n):
    ...
    for i in range(lendog+1,lendog+n+1):
        ...
        c.execute("""SELECT esp_de_vie FROM race WHERE id_race = ?""", (dog[2],))
        esp_de_vie = (c.fetchone())[0]
        années = randint(0+(esp_de_vie//2)*(type_vendeur == 'SPA')*(randint(0,1)),esp_de_vie-(esp_de_vie//2)*(type_vendeur == 'Particulier')*(randint(0,1)))
        années = str(années)
        ...
        dog.append(age)                    #Age
        c.execute("""SELECT taille_moyenne FROM race WHERE id_race = ?""", (dog[2],))
    ...
    c.executemany("""INSERT INTO chien (id_chien,sexe,id_race,age,taille,pedigree,id_vendeur,prix) VALUES(?,?,?,?,?,?,?,?)""", tabdog)
    conn.commit()
