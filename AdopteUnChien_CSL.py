...
while appli_on:
    ...
    while enter_exit:
        ...
        if enter[0:4].lower() == 'del ':
            ...
            print("\nSi vous souhaitez voir une photo d'un chien, tapez 'photo ?'")
            print("- où ? est à remplacer par le numéro du chien")
            ...
            if end_enter.lower() == 'exit':
                ...
                ID=int(end_enter[6:11])
                if 1 <= ID <= (len(tabres)+1):
                    chiot=0
                    toto=tabres[ID-1]
                    tutu=[toto[3]]+[toto[4]]
                    if int(tutu[1][0:2])<=1:
                        chiot = 1
                    titi=Image.open(tutu[0]+str(chiot)+'.png')
                    titi.show()
                else:
                    print('\nLe numéro ne correspond à aucun de ces chiens')
